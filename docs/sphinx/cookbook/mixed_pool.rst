...
and a :class:`umpire::strategy::QuickPool` for larger allocations.

.. note::
   Because `DynamicPoolMap` has recently been deprecated, the implementation 
   for `MixedPool` recently changed from using `DynamicPoolMap` to using `QuickPool`
   instead. Although `DynamicPoolMap` is still supported, `QuickPool` is encouraged.
