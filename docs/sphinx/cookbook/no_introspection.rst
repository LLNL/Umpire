...
for allocations that come from a particular Allocator, you can turn off the
introspection and avoid the overhead of tracking the associated metadata.
...
allocations made from the pool, so the getCurrentSize() method will return 0:
