...
class PrimaryPoolTest : public ::testing::Test
{
  public:
    ...
    void SetUp() override
    {
      ...
      m_pool_name = std::string{"pool_test"}
      ...
      m_allocator = new umpire::Allocator(  rm.makeAllocator<Pool>(m_pool_name
...
TYPED_TEST(PrimaryPoolTest, BlocksStatistic)
{
  ...
  for (int i{0}; i < 2; ++i) {
    ASSERT_NO_THROW(
      allocs[i] = this->m_allocator->allocate(this->m_initial_pool_size/2);
    );
  }
  ...
  for (int i{2}; i <= 3; ++i) {
    ASSERT_NO_THROW(
      allocs[i] = this->m_allocator->allocate(this->m_min_pool_growth_size/8);
    );
  }
...
TYPED_TEST(PrimaryPoolTest, coalesce)
{
  ...
  ASSERT_NO_THROW(
    {
      ptr_one = this->m_allocator->allocate( 1 +
                    this->m_initial_pool_size - this->m_min_pool_growth_size );
