...
void report_and_deallocate(int iteration, std::vector<void*>& ptrs, umpire::Allocator& allocator, T strategy)
{
  ...
  std::cout << std::setw(22) << allocator.getName() << ": " << iteration << std::setw(12) << current_size
            << " allocated across " << std::setw(4) << ptrs.size() << " allocations."
            << " Actual Size: " << std::setw(12) << actual_size << " High Watermark: " << std::setw(12)
            << high_watermark << " Adjusted - Current Size: " << std::setw(12) << allocator.getCurrentSize()
            << " Adjusted - Actual Size: " << std::setw(12) << allocator.getActualSize() << std::endl;
}

void allocate_and_check(std::size_t nbytes, std::size_t alignment, std::vector<void*>& ptrs,
                        umpire::Allocator& allocator)
{
  void* ptr{allocator.allocate(nbytes)};

  if (0 != (reinterpret_cast<std::ptrdiff_t>(ptr) % alignment)) {
    std::cerr << "Request for aligned pointer from " << allocator.getName() << " resulted in unaligned pointer. ("
              << ptr << ")" << std::endl;
    exit(1);
  }
...
int main()
{
  ...
  auto list_aligned_allocator = rm.makeAllocator<umpire::strategy::AlignedAllocator>(
      "HOST_list_aligned", list_allocation_pool, allocation_alignment);
  auto list_alloc =
      rm.makeAllocator<umpire::strategy::ThreadSafeAllocator>("HOST_list_safe_pool", list_aligned_allocator);
  ...
  for (int j{0}; j < max_iterations; ++j) {
    ...
    try {
      for (int i{0}; i < max_allocations && total_bytes <= max_total_size; ++i) {
        ...
        if (run_list) {
          allocate_and_check(nbytes, allocation_alignment, list_allocations, list_alloc);
        }
