...
struct copy<resource::host_platform, resource::host_platform> {
  ...
  // Specialization for void*
  template <>
  void exec<void>(void* src, void* dst, std::size_t len)
  {
    copy_impl<void>(src, dst, len);
  }

  template <>
  camp::resources::EventProxy<camp::resources::Resource> exec<void>(void* src, void* dst, std::size_t len,
                                                                    camp::resources::Resource& r)
  {
    copy_impl<void>(src, dst, len);
    return camp::resources::EventProxy<camp::resources::Resource>{r};
...
struct memset<resource::host_platform> {
  ...
  // Specialization for void*
  template <>
  void exec<void>(void* src, int val, std::size_t len)
  {
    memset_impl<void>(src, val, len);
  }

  template <>
  camp::resources::EventProxy<camp::resources::Resource> exec<void>(void* src, int val, std::size_t len,
                                                                    camp::resources::Resource& r)
  {
    memset_impl<void>(src, val, len);
    return camp::resources::EventProxy<camp::resources::Resource>{r};
...
struct reallocate<resource::host_platform> {
  ...
  static T* exec(T* src, std::size_t size)
  {
    ...
    if (!ret && size > 0) {
      UMPIRE_ERROR(runtime_error, fmt::format("Host realloc failed for pointer={}, size={}", src, size * sizeof(T)));
    }
  ...
  template <>
  void* exec<void>(void* src, std::size_t size)
  {
    if (!src) {
      return nullptr;
    }
    ...
    if (!ret && size > 0) {
      UMPIRE_ERROR(runtime_error, fmt::format("Host realloc failed for pointer={}, size={}", src, size));
    }
...
struct prefetch<resource::host_platform> {
  ...
  // Specialization for void*
  template <>
  void exec<void>(void* src, int device, std::size_t len)
  {
    // No-op for host memory
  }

  template <>
  camp::resources::EventProxy<camp::resources::Resource> exec<void>(void* src, int device, std::size_t len,
                                                                    camp::resources::Resource& r)
  {
    // No-op for host memory
    return camp::resources::EventProxy<camp::resources::Resource>{r};
