...
//////////////////////////////////////////////////////////////////////////////
#ifndef UMPIRE_resource_hip_device_const_memory_HPP
#define UMPIRE_resource_hip_device_const_memory_HPP
...
//! \brief Maximum total size, in bytes, of the fixed HIP `__constant__` buffer.
inline constexpr std::size_t hip_device_const_max_size = 64 * 1024;

namespace detail {

/*!
 * \brief Fixed-size HIP device constant-memory buffer.
 *
 * Declared `inline` (C++17) so that every translation unit including this
 * header shares exactly one definition of the symbol. This mirrors the
 * single file-scope `__constant__` buffer used by the v1
 * `HipConstantMemoryResource` (`src/umpire/resource/HipConstantMemoryResource.cpp`),
 * except the v1 version lived in exactly one translation unit by
 * construction (a single HIP-language source file) rather than a header.
 */
__constant__ inline char hip_device_const_buffer[hip_device_const_max_size];

} // namespace detail

/*!
 * \brief Unused allocator placeholder for `hip_device_const_memory`.
 *
 ...
 * called by `hip_device_const_memory`.
 */
struct hip_device_const_allocator {
...
/*!
 * \brief API v2 resource for HIP `__constant__` device memory.
 *
 * Faithfully ports the v1 `HipConstantMemoryResource` bump-offset
 ...
 */
template<
  typename Allocator = hip_device_const_allocator,
  bool Tracking = true
>
class hip_device_const_memory : public memory_resource<hip_platform, Allocator, Tracking> {
private:
  ...
  // Singleton instance
  static hip_device_const_memory& instance() {
    static hip_device_const_memory inst;
    return inst;
  }

  // Private constructor for singleton
  hip_device_const_memory()
    : base("DEVICE_CONST")
  {}
  ...
  void ensure_initialized() {
    ...
    hipError_t error = ::hipGetSymbolAddress(&ptr_, detail::hip_device_const_buffer);
    if (error != hipSuccess) {
      UMPIRE_ERROR(runtime_error,
                   fmt::format("hipGetSymbolAddress failed with error: {}",
...
public:
  //! \brief Return the default singleton for HIP constant memory.
  static hip_device_const_memory& get() {
    return instance();
  }

  /*!
   * \brief Construct a named HIP constant-memory resource.
   *
   ...
   * for diagnostics only, matching v1 (where each `HipConstantMemoryResource`
   * instance also bound to the same file-scope buffer).
   *
   * \param name Human-readable resource name.
   */
  explicit hip_device_const_memory(const std::string& name)
    : base(name)
  {}

  /*!
   * \brief Allocate from the fixed HIP constant-memory buffer.
   *
  ...
  void* allocate(std::size_t bytes) override {
    ...
    if (new_offset > hip_device_const_max_size) {
      UMPIRE_ERROR(runtime_error,
  ...
  /*!
   * \brief Deallocate from the fixed HIP constant-memory buffer.
   *
...
//! \brief Tracking-enabled HIP constant memory resource alias.
using default_hip_device_const_memory = hip_device_const_memory<hip_device_const_allocator, true>;
//! \brief HIP constant memory resource alias with tracking disabled.
using fast_hip_device_const_memory = hip_device_const_memory<hip_device_const_allocator, false>;
...
#endif // UMPIRE_resource_hip_device_const_memory_HPP
