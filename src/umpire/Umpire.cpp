...
void print_allocator_records(Allocator allocator, std::ostream& os)
{
  ...
  if (level != IntrospectionLevel::On) {
    UMPIRE_ERROR(runtime_error,
                 fmt::format("print_allocator_records requires introspection level \"on\". Current level is \"{}\".",
                             to_string(level)));
  }
...
std::vector<util::AllocationRecord> get_allocator_records(Allocator allocator)
{
  ...
  if (level != IntrospectionLevel::On) {
    UMPIRE_ERROR(runtime_error,
                 fmt::format("get_allocator_records requires introspection level \"on\". Current level is \"{}\".",
                             to_string(level)));
  }

  auto strategy = allocator.getAllocationStrategy();
  return rm.getTrackedAllocationRecords(strategy);
}
