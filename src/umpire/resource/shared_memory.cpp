...
/*!
 * \brief Segment storage manager: a straight port of v1's
 * `HostSharedMemoryResource::impl` (see
 * `src/umpire/resource/HostSharedMemoryResourceImpl.hpp`), adapted to
 * report block-creation/block-release outcomes back to `shared_memory` so
 * it can decide when to (un)track allocations in the API v2 registry.
 *
 * Segment layout:
 *
 *   [ SharedMemorySegmentHeader ][ SharedMemoryBlock ][ name ][ memory ] ...
 *
 * Blocks are linked via *offsets* relative to the mapped segment base
 * (rather than raw pointers), since the segment can be mapped at different
 * virtual addresses in different processes attached to it.
 */
class shared_memory::impl {
...
public:
  //! \brief Result of an allocate_named() call.
  struct allocation_result {
    void* ptr;      //!< Pointer to the allocation's memory (nullptr on failure).
    bool created;   //!< True if a new block was created (vs. attaching to an existing one).
  };

  impl(const std::string& name, std::size_t size)
  {
    m_segment_name = (!name.empty() && name[0] != '/') ? std::string{"/"} + name : name;
    ...
    //
    // SIMPLIFYING ASSUMPTION (ported from v1):
    //
    // This implementation depends upon something else cleaning up what is
    // placed in /dev/shm (on Linux) or the equivalent macOS shared memory
    // namespace. It is the caller's responsibility to ensure stale segments
    // from previous runs are cleaned up (e.g. via an epilogue script, or by
    // choosing a name unlikely to collide with a stale segment).
    //
    ...
    if (created) {
      if (0 != ftruncate(m_segment_fd, static_cast<off_t>(size))) {
        err = errno;
  ...
  ~impl()
  {
    ...
    if (munmap(m_segment, m_segment_size) != 0) {
      err = errno;
  ...
  allocation_result allocate_named(const std::string& name, std::size_t requested_size)
  {
    ...
    // First let's see if the allocation already exists
    ...
    return allocation_result{ptr, created};
  }

  //! \return true if the block backing `ptr` was fully released (reference
  //! count reached zero and it was returned to the free list).
...
private:
  ...
  SharedMemoryBlock* find_existing_allocation(const std::string& name)
  {
    SharedMemoryBlock* block_ptr;
  ...
  void map_shared_memory_segment()
  {
    ...
    m_segment_size = static_cast<std::size_t>(size);
  }
};

namespace {
std::atomic<unsigned long long> g_anonymous_counter{0};
}

shared_memory::shared_memory(const std::string& name, std::size_t size, bool tracking)
  : umpire::memory(name)
...
void* shared_memory::allocate(std::size_t bytes)
{
  if (bytes == 0) {
    return nullptr;
  }

  // shared_memory only knows how to store named blocks; synthesize a
  // unique internal name for anonymous allocations (mirroring v1's
  // NamingShim pattern) rather than rejecting the call outright.
  std::string anon_name{
      fmt::format("{}_alloc_{}", get_name(), g_anonymous_counter.fetch_add(1, std::memory_order_relaxed))};

  return allocate(anon_name, bytes);
}

void* shared_memory::allocate(const std::string& name, std::size_t bytes)
{
  impl::allocation_result result{pimpl_->allocate_named(name, bytes)};

  UMPIRE_LOG(Debug, "(name=\"" << name << ", requested_size=" << bytes << ") returning: " << result.ptr);

  // Only track allocations for blocks newly created through this call;
  // attaching to an already-existing named block (result.created == false)
  // does not double-count bytes already tracked when the block was first
  // created.
  if (tracking_ && result.created && result.ptr != nullptr) {
    track_allocation(result.ptr, bytes);
  }

  return result.ptr;
}

void shared_memory::deallocate(void* ptr)
{
  if (!ptr) {
    return;
  }

  bool released{pimpl_->deallocate(ptr)};

  // Symmetric with allocate(): only untrack once the block's reference
  // count has actually reached zero and it has been returned to the
  // segment's free list.
  if (tracking_ && released) {
    untrack_allocation(ptr);
  }
}

void* shared_memory::find_pointer_from_name(const std::string& name)
{
  return pimpl_->find_pointer_from_name(name);
}

resource::Platform shared_memory::get_platform() const
{
  return resource::Platform::host;
}

std::size_t shared_memory::get_segment_actual_size() const
{
  return pimpl_->getActualSize();
}
