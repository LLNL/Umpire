  ...
  impl(const std::string& name, std::size_t size)
  {
    ...
    if (created) {
      if (0 != ftruncate(m_segment_fd, size)) {
        err = errno;
        UMPIRE_ERROR(runtime_error,
                     "Failed to set size for shared memory segment " << m_segment_name << ": " << strerror(err));
      }
      ...
      if ((err = pthread_mutexattr_init(&mattr)) != 0) {
        UMPIRE_ERROR(runtime_error, "Failed to initialize mutex attributes for shared memory segment "
                                        << m_segment_name << ": " << strerror(err));
      }

      if ((err = pthread_mutexattr_setpshared(&mattr, PTHREAD_PROCESS_SHARED)) != 0) {
        UMPIRE_ERROR(runtime_error, "Failed to set shared atributes for mutex for shared memory segment "
                                        << m_segment_name << ": " << strerror(err));
      }
