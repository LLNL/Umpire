 ...
 public:
  impl(const std::string& name, std::size_t size)
  {
    ...
    while (!completed) { // spin on opening shm
      if (open_shared_memory_segment(err, (O_RDWR | O_CREAT | O_EXCL))) {
        ...
        UMPIRE_ERROR(runtime_error, fmt::format("Failed to create shared memory segment \"{}\": {}",
        ...
        if (open_shared_memory_segment(err, O_RDWR)) {
          ...
          UMPIRE_ERROR(runtime_error, fmt::format("Failed to open shared memory file \"{}\": {}",
                                                  m_segment_name, strerror(err)));
        }
      }
    ...
    if (created) {
      ...
      if ((err = pthread_mutexattr_setpshared(&mattr, PTHREAD_PROCESS_SHARED)) != 0) {
        UMPIRE_ERROR(runtime_error,
                     fmt::format("Failed to set shared attributes for shared memory segment \"{}\": {}",
                                 m_segment_name, strerror(err)));
      }

      if ((err = pthread_mutex_init(&m_segment->mutex, &mattr)) != 0) {
        UMPIRE_ERROR(runtime_error,
                     fmt::format("Failed to initialize mutex for shared memory segment \"{}\": {}",
                                 m_segment_name, strerror(err)));
      }
      ...
      while (filesize == 0) {
        ...
        if (fstat(m_segment_fd, &st) < 0) {
          err = errno;
          UMPIRE_ERROR(runtime_error, fmt::format("Failed fstat for shared memory segment  {}: {}",
                                                  m_segment_name, strerror(err)));
        }
        filesize = st.st_size;
        std::this_thread::yield();
  ...
  void* allocate_named(const std::string& name, std::size_t requested_size)
  {
    ...
    if ((err = pthread_mutex_lock(&m_segment->mutex)) != 0) {
      UMPIRE_ERROR(runtime_error, fmt::format("Failed to lock mutex for shared memory segment {}: {}",
                                              m_segment_name, strerror(err)));
    }
 ...
 private:
  ...
  void map_shared_memory_segment()
  {
    ...
    if (0 != fstat(m_segment_fd, &buf)) {
      int err = errno;
      UMPIRE_ERROR(runtime_error, fmt::format("Failed to obtain size of shared object \"{}\": {}",
                                              m_segment_name, strerror(err)));
    }
    ...
    if (base == MAP_FAILED) {
      int err = errno;
      UMPIRE_ERROR(runtime_error,
                   fmt::format("Failed to map shared object \"{}\": {}", m_segment_name, strerror(err)));
    }
