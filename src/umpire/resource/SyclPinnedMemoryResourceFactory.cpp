...
bool
SyclPinnedMemoryResourceFactory::isValidMemoryResourceFor(const std::string& name)
  noexcept
{
...
SyclPinnedMemoryResourceFactory::getDefaultTraits()
{
  ...
  if (sycl_device.is_gpu() && (deviceName.find("Intel(R) Gen9 HD Graphics NEO") != std::string::npos)) {
    traits.size = 0; //sycl_device.get_info<cl::sycl::info::device::global_mem_size>(); // in bytes
