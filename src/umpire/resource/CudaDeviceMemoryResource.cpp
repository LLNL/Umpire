...
void* CudaDeviceMemoryResource::allocate(std::size_t bytes)
{
  ...
  if (err != cudaSuccess) {
    UMPIRE_ERROR(runtime_error,
                 fmt::format("cudaGetDevice( device = {} ) failed with error: {}", old_device, cudaGetErrorString(err)));
  }
  if (old_device != m_traits.id) {
    err = cudaSetDevice(m_traits.id);
    if (err != cudaSuccess) {
      UMPIRE_ERROR(runtime_error,
                   fmt::format("cudaSetDevice( device = {} ) failed with error: {}", m_traits.id, cudaGetErrorString(err)));
    }
  }
  ...
  if (old_device != m_traits.id) {
    ...
    if (err != cudaSuccess) {
      UMPIRE_ERROR(runtime_error,
                   fmt::format("cudaSetDevice( device = {} ) failed with error: {}", old_device, cudaGetErrorString(err)));
    }
  }
  return ptr;
}
