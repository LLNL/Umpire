                                                 ...
                                                 std::size_t length)
{
  ...
  if (properties.managedMemory == 1 && properties.concurrentManagedAccess == 1) {
    ...
    if (error != cudaSuccess) {
      UMPIRE_ERROR(runtime_error, umpire::fmt::format("cudaMemAdvise( src_ptr = {}, length = {}, cudaMemAdviseSetPreferredLocation, device = {}) failed with error: ", src_ptr, length, device, cudaGetErrorString(error)));
    }
  }
}
