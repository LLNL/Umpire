                                     ...
                                     util::AllocationRecord* UMPIRE_UNUSED_ARG(dst_allocation), std::size_t length)
{
  ...
  if (error != hipSuccess) {
    UMPIRE_ERROR(
        runtime_error,
        umpire::fmt::format(
            "hipMemcpy( dest_ptr = {}, src_ptr = {}, length = {}, hipMemcpyDeviceToHost) failed with error: {}",
    ...
    util::AllocationRecord* UMPIRE_UNUSED_ARG(dst_allocation), std::size_t length, camp::resources::Resource& ctx)
{
  ...
  if (error != hipSuccess) {
    UMPIRE_ERROR(runtime_error,
                 umpire::fmt::format("hipMemcpyAsync( dest_ptr = {}, src_ptr = {}, length = {}, hipMemcpyDeviceToHost, "
