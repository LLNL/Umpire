                                 ...
                                 umpire::util::AllocationRecord* UMPIRE_UNUSED_ARG(dst_allocation), std::size_t length)
{
  ...
  if (error != hipSuccess) {
    UMPIRE_ERROR(runtime_error, umpire::fmt::format("hipMemcpy( dest_ptr = {}, src_ptr = {}, length = {}, hipMemcpyDeviceToDevice) failed with error: {}", *dst_ptr, src_ptr, length, hipGetErrorString(error)));
  }
}
    ...
    util::AllocationRecord* UMPIRE_UNUSED_ARG(dst_allocation), std::size_t length, camp::resources::Resource& ctx)
{
  ...
  if (error != hipSuccess) {
    UMPIRE_ERROR(runtime_error, umpire::fmt::format("hipMemcpyAsync( dest_ptr = {}, src_ptr = {}, length = {}, hipMemcpyDeviceToDevice, stream = {}) failed with error: {}", *dst_ptr, src_ptr, length, hipGetErrorString(error), (void*)stream));
  }
