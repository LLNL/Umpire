...
#define UMPIRE_SET_UP_DEVICE_ALLOCATORS()                                                                              \
  {                                                                                                                    \
    if (umpire::macro_tracking == 0) {                                                                                 \
      ...
      if (err != cudaSuccess) {                                                                                        \
        UMPIRE_ERROR(runtime_error, fmt::format("cudaMemcpyToSymbol failed with error: {}", cudaGetErrorString(err))); \
      }                                                                                                                \
    }                                                                                                                  \
  ...
  {                                                                                                                  \
    if (umpire::macro_tracking == 0) {                                                                               \
      ...
      hipError_t err = hipMemcpyToSymbol(umpire::UMPIRE_DEV_ALLOCS, &umpire::UMPIRE_DEV_ALLOCS_h,                    \
                                         sizeof(umpire::DeviceAllocator*));                                          \
      if (err != hipSuccess) {                                                                                       \
        UMPIRE_ERROR(runtime_error, fmt::format("hipMemcpyToSymbol failed with error: {}", hipGetErrorString(err))); \
      }                                                                                                              \
    }                                                                                                                \
    umpire::macro_tracking = 1;                                                                                      \
  }
