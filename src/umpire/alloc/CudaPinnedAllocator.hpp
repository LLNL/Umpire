...
struct CudaPinnedAllocator {
  void* allocate(std::size_t bytes)
  {
    ...
    if (error != cudaSuccess) {
      if (error == cudaErrorMemoryAllocation) {
        UMPIRE_ERROR(out_of_memory_error, fmt::format("cudaMalloc( bytes = {} ) failed with error: {}", bytes,
                                                      cudaGetErrorString(error)));
      } else {
        UMPIRE_ERROR(runtime_error, fmt::format("cudaMalloc( bytes = {} ) failed with error: {}", bytes,
                                                cudaGetErrorString(error)));
      }
    }
  ...
  void deallocate(void* ptr)
  {
    ...
    if (error != cudaSuccess) {
      UMPIRE_ERROR(runtime_error, fmt::format("cudaFreeHost( ptr = {} ) failed with error: {}", ptr,
                                              cudaGetErrorString(error)));
    }
  }
