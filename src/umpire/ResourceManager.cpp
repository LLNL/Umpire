...
int ResourceManager::getNumDevices() const
{
  ...
  if (err != cudaSuccess) {
    UMPIRE_ERROR(runtime_error, fmt::format("Error: cudaGetDeviceCount failed with error: {}", cudaGetErrorString(err)));
  }
#elif defined(UMPIRE_ENABLE_HIP)
  hipError_t err = hipGetDeviceCount(&device_count);
  if (err != hipSuccess) {
    UMPIRE_ERROR(runtime_error,
                 fmt::format("Error: hipGetDeviceCount failed with error: {}", hipGetErrorString(error)));
  }
